    ...
    public IEnumerator PlayMusic(AudioClip clip)
    {
        if (currentAudioSource.clip != null)
        {
            ...
            while (currentAudioSource.isPlaying)
            {
                ...
                {
                    yield return instance.StartCoroutine(FadeVolume(currentAudioSource, instance.fadeDuration, MusicVolume));
                }
                if (currentAudioSource.clip.length - currentAudioSource.time <= instance.timeToStartDownFade)
                {
                    yield return instance.StartCoroutine(FadeVolume(currentAudioSource, instance.fadeDuration, 0));
        ...
        while (currentAudioSource.isPlaying)
        {
            if (currentAudioSource.time <= instance.timeToStartUpperFade)
            {
                yield return instance.StartCoroutine(FadeVolume(currentAudioSource, MusicVolume <= 0.0f ? 0.0f : fadeDuration, MusicVolume));
            }
            if (currentAudioSource.clip.length - currentAudioSource.time <= timeToStartDownFade)
            {
                yield return instance.StartCoroutine(FadeVolume(currentAudioSource, fadeDuration, 0));
                currentAudioSource.Stop();
                currentAudioSource.clip = null;
            }
            yield return null;
